...
import com.github.chistousov.readonly_backend.SpringSecurityConfiguration;
...
public class DataControllerTest {
  ...
  private static final String SESSION_CLIENT_READONLY = "SESSION";
  ...
  public static void settings(DynamicPropertyRegistry registry)
    ...
    registry.add("spring.security.oauth2.client.registration.client-readonly.client-id", () -> clientId);
    registry.add("spring.security.oauth2.client.registration.client-readonly.client-secret", () -> clientSecret);

    redirectUri = UriComponentsBuilder
        .fromUri(URI.create(baseHostPath))
        .pathSegment("login", "oauth2", "code", "client-readonly")
        .build()
        .toUriString();

    registry.add("spring.security.oauth2.client.registration.client-readonly.redirect-uri", () -> redirectUri);
    ...
    registry.add("server.reactive.session.cookie.name", () -> SESSION_CLIENT_READONLY);
  }
  ...
  void testGetStatistics() {
    ...
    thisServerWebTestClient
        ...
        .exists("Location")
        .expectHeader()
        .valueMatches("Location", "/oauth2/authorization/client-readonly");

    thisServerWebTestClient
        .get()
        .uri("/oauth2/authorization/client-readonly")
        .exchange()
        .expectStatus()
        .isFound()
        .expectHeader()
        ...
        .expectCookie()
        .exists(SESSION_CLIENT_READONLY)
        .expectCookie()
        .value(SESSION_CLIENT_READONLY, val -> cookieThisSession = val);
    ...
    thisServerWebTestClient
        ...
        .uri(
            ...
            uriBuilder -> uriBuilder
                .path("/login/oauth2/code/client-readonly")
                .queryParam("code", code)
                .queryParam("state", state)
                .build())
        .cookie(SESSION_CLIENT_READONLY, cookieThisSession)
        .exchange()
        .expectStatus()
        .isFound()
        .expectHeader()
  ...
  @Test
  @DisplayName("/data is fauil. Token is incorrect")
  void testGetStatisticsTokenIncorrect() {
    // given (instead of when)
