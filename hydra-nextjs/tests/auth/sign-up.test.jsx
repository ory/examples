  ...
  describe('POST', () => {
    ...
    it('Rejects emails already in use', async () => {
      ...
      const req = mockRequest({
        ...
        body: 'password=x&password_confirm=x&email=' + encodeURIComponent('foo@bar.com')
      });
      const res = mockResponse();
      const query = {login_challenge: 'valid'};
      const result = await getServerSideProps({ req, res, query });
    ...
    it('Accepts valid submission', async() => {
      ...
      const req = mockRequest({
        ...
        body: 'remember=1&password=x&password_confirm=x&email=' + encodeURIComponent('email@example.com')
      });
      const res = mockResponse();
      const query = {login_challenge: 'valid'};
      const result = await getServerSideProps({ req, res, query });
